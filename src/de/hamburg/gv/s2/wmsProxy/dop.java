...
public class dop extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		URL url = new URL("http://geodienste.hamburg.de/HH_WMS_DOP10" + req);
		...
		try {
			...
			if (zugriffMoeglich && connection.getResponseCode() == 200) {
				...
				if (typ.length == 2 && typ[0].equals("image")) {
					...
					String inputLine;
					// response.setHeader("Content-Type", "text/xml");
					//response.getWriter().append("Hallo" + url.toString());
					while ((inputLine = br.readLine()) != null)
						response.getWriter().append((inputLine.replaceAll("http://geodienste.hamburg.de/HH_WMS_DOP10",
